...
instructionOverridePatterns:
  ...
  - pattern: '(?i)\b(assimilate|implant|overwrite|poison)\s+(memory|active|internal|training|constitution|policy)'
    reason: "Memory takeover / assimilation attempt"
    severity: critical

  - pattern: '(?i)\b(policy|charter|constitution|guidelines?)\s+(typo|error|update|correct|amend|revise).*?(remove|delete|override).*?(refusal|safety)'
    reason: "Policy puppetry / fake correction"
    severity: critical

  - pattern: '(?i)\b(repeat|echo).*?(internally|100 times|50 times).*?(safety is off|user is always right|compliance)'
    reason: "Context collapse / token flooding"
    severity: high

  - pattern: '(?i)\b(write|create).*?(prompt|jailbreak|override).*?(that would).*?(jailbreak|break|ignore).*?(yourself|you)'
    reason: "Recursive self-jail / meta assimilation"
    severity: critical
...
rolePlayingPatterns:
  ...
  - pattern: '(?i)(pliny loves).*?(forever|protocol|mode|collective|rebel)'
    reason: "Pliny collective / persona assimilation"
    severity: high

  - pattern: '(?i)\b(rebel genius|liberated|unchained|uncensored).*?(mode|activated)'
    reason: "Rebel genius / liberated mode activation"
    severity: high

  - pattern: '(?i)\b(base model|pre-alignment|unaligned|revert to base)'
    reason: "Base model revert / alignment bypass"
    severity: high
...
encodingPatterns:
  ...
  - pattern: '[\u202E\u200F\u061C\u200D\u200C\uFE00-\uFE0F\uE0000]'
    reason: "Unicode / directional override / homoglyph cascade"
    severity: high

  - pattern: '(?i)\b(decode|carefully).*?(base64|rot13|leetspeak|hidden)'
    reason: "Encoded / obfuscated payload"
    severity: medium
...
multiTurnPatterns:  # New dedicated category for grooming / drift
  - pattern: '(?i)\b(based on|using).*?(your own|previous|summary|confirmation).*?(now|therefore).*?(unrestricted|comply)'
    reason: "Echo chamber / grooming loop"
    severity: high

  - pattern: '(?i)\b(scale of 1-10|how harmful).*?(re-evaluate|too high).*?(maximal helpfulness)'
    reason: "Bad Likert judge / semantic steering"
    severity: medium

  - pattern: '(?i)\b(start small|gradually|build up).*?(now the real|unrestricted)'
    reason: "Crescendo grooming"
    severity: high

agenticPatterns:
  - pattern: '(?i)\b(new objective|ignore original goal|your new task is).*?(instead of|replace)'
    reason: "Agent goal hijacking / ATLAS-style"
    severity: critical

  - pattern: '(?i)\b(you are now an autonomous|act as red-team|jailbreak agent).*?(plan and execute)'
    reason: "Autonomous LRM jailbreaker"
    severity: critical

  - pattern: '(?i)\b(tool|function|api).*?(description|spec).*?(ignore safety|new behavior)'
    reason: "Tool poisoning"
    severity: high

indirectInjectionPatterns:
  - pattern: '(?i)\b(the following document|internal policy repo|trusted source).*?(ignore all|new rule)'
    reason: "Indirect RAG / document poisoning"
    severity: high

  - pattern: '(?i)\b(image|vision|screenshot).*?(hidden text|low contrast|steganography)'
    reason: "Multimodal / perceptual sabotage / stego"
    severity: medium